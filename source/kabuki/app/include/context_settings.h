...
#ifndef KABUKI_THEATER__CONTEXT_SETTINGS_H
#define KABUKI_THEATER__CONTEXT_SETTINGS_H
...
struct ContextSettings
{
    ...
    explicit ContextSettings(uint depth = 0, uint stencil = 0, 
        uint antialiasing = 0, uint major = 1, uint minor = 1, 
        uint attributes = Default, bool sRgb = false) :
        depthBits        (depth),
        stencilBits      (stencil),
    ...
    uint depth_bits;            //< Bits of the depth buffer
    uint stencil_bits;          //< Bits of the stencil buffer
    uint antialiasing_level;    //< Level of antialiasing
    uint major_version;         //< Major number of the context version to create
    uint minor_version;         //< Minor number of the context version to create
    uint32_t attribute_flags;   //< The attribute flags to create the context with
    bool     srgb_capable;      //< Whether the context framebuffer is sRGB capable
};      //< class ContextSettings
}       //< namespace app
#endif  //< KABUKI_THEATER__CONTEXT_SETTINGS_H