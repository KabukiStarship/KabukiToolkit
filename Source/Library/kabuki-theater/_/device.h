...
#ifndef CHINESEROOM_DEVICE_H
#define CHINESEROOM_DEVICE_H
...
INLINE uintptr_t ToUInt (Device* d) {
...
INLINE bool IsDevice (const Member* member) {
    if (!member)
        return false;
...
INLINE void PrintAddress (const byte* address, Device* root) {
    if (address == nullptr)
        return;
    if (root == nullptr)
        return;
