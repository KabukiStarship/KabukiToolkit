...
static int stbi__dds_info(stbi__context *s, int *x, int *y, int *comp,
  ...
  *y = header.dwHeight;
  ...
  if (iscompressed) *iscompressed = is_compressed;

  return 1;
}

int stbi__dds_info_from_memory(stbi_uc const *buffer, int len, int *x, int *y,
...
int stbi__dds_info_from_callbacks(stbi_io_callbacks const *clbk, void *user,
...
#ifndef STBI_NO_STDIO
int stbi__dds_info_from_path(char const *filename, int *x, int *y, int *comp,
                             int *iscompressed) {
  int res;
  FILE *f;
  errno_t err = fopen_s(&f, filename, "rb");
...
int stbi__dds_info_from_file(FILE *f, int *x, int *y, int *comp,
                             int *iscompressed) {
  stbi__context s;
  int res;
  long n = ftell(f);
...
static void *stbi__dds_load(stbi__context *s, int *x, int *y, int *comp,
                            int req_comp) {
  ...
  if (is_compressed) {
    ...
    *comp = s->img_n;
    ...
    for (cf = 0; cf < cubemap_faces; ++cf) {
      ...
      stbi__getn(s, &dds_data[cf * s->img_x * s->img_y * s->img_n],
  ...
  *y = s->img_y;
  ...
  if ((req_comp <= 4) && (req_comp >= 1)) {
    ...
    if (req_comp != s->img_n) {
      dds_data = stbi__convert_format(dds_data, s->img_n, req_comp, s->img_x,
                                      s->img_y);
      *comp = req_comp;
    }
  } else {
...
#ifndef STBI_NO_STDIO
void *stbi__dds_load_from_file(FILE *f, int *x, int *y, int *comp,
                               int req_comp) {
  stbi__context s;
  stbi__start_file(&s, f);
...
void *stbi__dds_load_from_path(const char *filename, int *x, int *y, int *comp,
                               int req_comp) {
  void *data;
  FILE *f;
  errno_t err = fopen_s(&f, filename, "rb");
...
void *stbi__dds_load_from_memory(stbi_uc const *buffer, int len, int *x, int *y,
...
void *stbi__dds_load_from_callbacks(stbi_io_callbacks const *clbk, void *user,
