/** Kabuki Software Development Kit
    @file    /.../KabukiSDK-Library/_G/Polygon.hpp
...
/*< A 2D polygon that uses 32-bit floating point values. 
    This class is a C++ implementation of Java's polygon class that does not 
    use polymorphism.
...
*/
{
    ...
    enum {
        MinNumPoints = 3,           //< The minimum number of points in a Polygon; which is a triangle.
    ...
    Polygon_f (float* xPoints, float* yPoints, int aNumPoints, int aBufferSize);
    ...
    PolygonPathIterator_f getPathIterator (AffineTransform_f a);

    PolygonPathIterator_f getPathIterator (AffineTransform_f a, float flatness);
...
class PolygonPathIterator_f
{
    public:

    PolygonPathIterator_f (Polygon_f p, AffineTransform_f a);
