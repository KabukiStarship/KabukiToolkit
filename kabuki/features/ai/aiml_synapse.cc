...
@file    /kabuki/f2sdk/ai/aiml_synapse.cc
...
Synapse::Synapse(Perceptron* rx) : rx_(rx), weight_(SynapseNoise()) {
  // Nothing to do here! :-)
}

Synapse::Synapse(Perceptron* rx, float_t weight) : rx_(rx), weight_(weight) {
  // Nothing to do here! :-)
}
...
Synapse::Synapse(const Synapse& other)
    : weight_(other.weight_), rx_(other.rx_) {
  // Nothing to do here! :-)
}
