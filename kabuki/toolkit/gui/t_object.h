...
public:
  ...
  // Copy constructor
  Object (const Object & /*unused*/) : m_refCount (0) {}
  ...
  The object will automatically be deallocated once the reference count reaches 
  zero. */
...
takes care of increasing and decreasing the object's reference count as needed.
When the last reference goes out of scope, the associated object will be
deallocated.

The advantage over C++ solutions such as `std::shared_ptr` is that the 
reference count is very compactly integrated into the base object itself. */
