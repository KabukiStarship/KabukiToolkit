...
@file    /kabuki/features/hmi/hmi_mixer.cc
...
Mixer::Mixer()
  ...
  for (int i = 1; i < defaultMixerArraySize; ++i) mixer[i] = 0;
}

Mixer::~Mixer() {
  for (int i = 0; i < num_channels_ + 1; ++i) delete mixer[i];
  delete mixer;
}

int Mixer::GetVolume(int channelNum) { return mixer[channelNum]->getVolume(); }

int Mixer::GetPan(int channelNum) { return mixer[channelNum]->getPan(); }
...
int Mixer::isMuted(int channelNum) { return mixer[channelNum]->isMuted(); }

int Mixer::isSoloed(int channelNum) { return mixer[channelNum]->isSoloed(); }
...
int Mixer::GetControlsResolution() { return controlResolution; }
...
const char* Mixer::SetState(byte Value) { return 0; }
