...
struct NVGglyphPosition {
  ...
  FPN x;            // The x-coordinate of the logical glyph position.
  FPN minx, maxx;   // The bounds of the glyph shape.
};
...
struct NVGtextRow {
  ...
  FPN width;          // Logical width of the row.
  FPN minx, maxx;     // Actual bounds of the row. Logical with and bounds can
...
void nvgBeginFrame(NVGcontext* ctx, FPN windowWidth, FPN windowHeight,
                   FPN devicePixelRatio);
...
// to 1.0f.
NVGcolor nvgRGBf(FPN r, FPN g, FPN b);
...
// Returns a color value from red, green, blue and alpha values.
NVGcolor nvgRGBAf(FPN r, FPN g, FPN b, FPN a);

// Linearly interpolates from color c0 to c1, and returns resulting color value.
NVGcolor nvgLerpRGBA(NVGcolor c0, NVGcolor c1, FPN u);

// Sets transparency of a color value.
NVGcolor nvgTransRGBA(NVGcolor c0, unsigned char a);

// Sets transparency of a color value.
NVGcolor nvgTransRGBAf(NVGcolor c0, FPN a);

// Returns color value specified by hue, saturation and lightness.
// HSL values are all in range [0..1], alpha will be set to 255.
NVGcolor nvgHSL(FPN h, FPN s, FPN l);

// Returns color value specified by hue, saturation and lightness and alpha.
// HSL values are all in range [0..1], alpha in range [0..255]
NVGcolor nvgHSLA(FPN h, FPN s, FPN l, unsigned char a);
...
void nvgMiterLimit(NVGcontext* ctx, FPN limit);

// Sets the stroke width of the stroke style.
void nvgStrokeWidth(NVGcontext* ctx, FPN size);
...
void nvgGlobalAlpha(NVGcontext* ctx, FPN alpha);
...
//   [b d f]
//   [0 0 1]
void nvgTransform(NVGcontext* ctx, FPN a, FPN b, FPN c, FPN d, FPN e, FPN f);

// Translates current coordinate system.
void nvgTranslate(NVGcontext* ctx, FPN x, FPN y);

// Rotates current coordinate system. Angle is specified in radians.
void nvgRotate(NVGcontext* ctx, FPN angle);

// Skews the current coordinate system along X axis. Angle is specified in
// radians.
void nvgSkewX(NVGcontext* ctx, FPN angle);

// Skews the current coordinate system along Y axis. Angle is specified in
// radians.
void nvgSkewY(NVGcontext* ctx, FPN angle);

// Scales the current coordinate system.
void nvgScale(NVGcontext* ctx, FPN x, FPN y);
...
void nvgCurrentTransform(NVGcontext* ctx, FPN* xform);

// The following functions can be used to make calculations on 2x3
// transformation matrices. A 2x3 matrix is represented as FPN[6].

// Sets the transform to identity matrix.
void nvgTransformIdentity(FPN* dst);

// Sets the transform to translation matrix matrix.
void nvgTransformTranslate(FPN* dst, FPN tx, FPN ty);

// Sets the transform to scale matrix.
void nvgTransformScale(FPN* dst, FPN sx, FPN sy);

// Sets the transform to rotate matrix. Angle is specified in radians.
void nvgTransformRotate(FPN* dst, FPN a);

// Sets the transform to skew-x matrix. Angle is specified in radians.
void nvgTransformSkewX(FPN* dst, FPN a);

// Sets the transform to skew-y matrix. Angle is specified in radians.
void nvgTransformSkewY(FPN* dst, FPN a);

// Sets the transform to the result of multiplication of two transforms, of A =
// A*B.
void nvgTransformMultiply(FPN* dst, const FPN* src);

// Sets the transform to the result of multiplication of two transforms, of A =
// B*A.
void nvgTransformPremultiply(FPN* dst, const FPN* src);

// Sets the destination to inverse of specified transform.
// Returns 1 if the inverse could be calculated, else 0.
int nvgTransformInverse(FPN* dst, const FPN* src);

// Transform a point by given transform.
void nvgTransformPoint(FPN* dstx, FPN* dsty, const FPN* xform, FPN srcx,
                       FPN srcy);

// Converts degrees to radians and vice versa.
FPN nvgDegToRad(FPN deg);
FPN nvgRadToDeg(FPN rad);
...
NVGpaint nvgLinearGradient(NVGcontext* ctx, FPN sx, FPN sy, FPN ex, FPN ey,
...
NVGpaint nvgBoxGradient(NVGcontext* ctx, FPN x, FPN y, FPN w, FPN h, FPN r,
                        FPN f, NVGcolor icol, NVGcolor ocol);
...
// nvgStrokePaint().
NVGpaint nvgRadialGradient(NVGcontext* ctx, FPN cx, FPN cy, FPN inr, FPN outr,
                           NVGcolor icol, NVGcolor ocol);
...
// gradient is transformed by the current transform when it is passed to
// nvgFillPaint() or nvgStrokePaint().
NVGpaint nvgImagePattern(NVGcontext* ctx, FPN ox, FPN oy, FPN ex, FPN ey,
                         FPN angle, int image, FPN alpha);
...
void nvgScissor(NVGcontext* ctx, FPN x, FPN y, FPN w, FPN h);
...
void nvgIntersectScissor(NVGcontext* ctx, FPN x, FPN y, FPN w, FPN h);
...
void nvgMoveTo(NVGcontext* ctx, FPN x, FPN y);

// Adds line segment from the last point in the path to the specified point.
void nvgLineTo(NVGcontext* ctx, FPN x, FPN y);

// Adds cubic bezier segment from last point in the path via two control points
// to the specified point.
void nvgBezierTo(NVGcontext* ctx, FPN c1x, FPN c1y, FPN c2x, FPN c2y, FPN x,
                 FPN y);

// Adds quadratic bezier segment from last point in the path via a control point
// to the specified point.
void nvgQuadTo(NVGcontext* ctx, FPN cx, FPN cy, FPN x, FPN y);

// Adds an arc segment at the corner defined by the last path point, and two
// specified points.
void nvgArcTo(NVGcontext* ctx, FPN x1, FPN y1, FPN x2, FPN y2, FPN radius);
...
void nvgArc(NVGcontext* ctx, FPN cx, FPN cy, FPN r, FPN a0, FPN a1, int dir);

// Creates new rectangle shaped sub-path.
void nvgRect(NVGcontext* ctx, FPN x, FPN y, FPN w, FPN h);

// Creates new rounded rectangle shaped sub-path.
void nvgRoundedRect(NVGcontext* ctx, FPN x, FPN y, FPN w, FPN h, FPN r);

// Creates new rounded rectangle shaped sub-path with varying radii for each
// corner.
void nvgRoundedRectVarying(NVGcontext* ctx, FPN x, FPN y, FPN w, FPN h,
                           FPN radTopLeft, FPN radTopRight, FPN radBottomRight,
                           FPN radBottomLeft);

// Creates new ellipse shaped sub-path.
void nvgEllipse(NVGcontext* ctx, FPN cx, FPN cy, FPN rx, FPN ry);

// Creates new circle shaped sub-path.
void nvgCircle(NVGcontext* ctx, FPN cx, FPN cy, FPN r);
...
void nvgFontSize(NVGcontext* ctx, FPN size);

// Sets the blur of current text style.
void nvgFontBlur(NVGcontext* ctx, FPN blur);

// Sets the letter spacing of current text style.
void nvgTextLetterSpacing(NVGcontext* ctx, FPN spacing);

// Sets the proportional line height of current text style. The line height is
// specified as multiple of font size.
void nvgTextLineHeight(NVGcontext* ctx, FPN lineHeight);
...
FPN nvgText(NVGcontext* ctx, FPN x, FPN y, const char* string, const char* end);
...
void nvgTextBox(NVGcontext* ctx, FPN x, FPN y, FPN breakRowWidth,
                const char* string, const char* end);

// Measures the specified text string. Parameter bounds should be a pointer to
// FPN[4], if the bounding box of the text should be returned. The bounds value
// are [xmin,ymin, xmax,ymax] Returns the horizontal advance of the measured
// text (i.e. where the next character should drawn). Measured values are
// returned in local coordinate space.
FPN nvgTextBounds(NVGcontext* ctx, FPN x, FPN y, const char* string,
                  const char* end, FPN* bounds);

// Measures the specified multi-text string. Parameter bounds should be a
// pointer to FPN[4], if the bounding box of the text should be returned. The
// bounds value are [xmin,ymin, xmax,ymax] Measured values are returned in local
// coordinate space.
void nvgTextBoxBounds(NVGcontext* ctx, FPN x, FPN y, FPN breakRowWidth,
                      const char* string, const char* end, FPN* bounds);
...
// coordinate space.
int nvgTextGlyphPositions(NVGcontext* ctx, FPN x, FPN y, const char* string,
                          const char* end, NVGglyphPosition* positions,
                          int maxPositions);

// Returns the vertical metrics based on the current text style.
// Measured values are returned in local coordinate space.
void nvgTextMetrics(NVGcontext* ctx, FPN* ascender, FPN* descender, FPN* lineh);
...
int nvgTextBreakLines(NVGcontext* ctx, const char* string, const char* end,
                      FPN breakRowWidth, NVGtextRow* rows, int maxRows);
...
struct NVGparams {
  ...
  void (*renderViewport)(void* uptr, FPN width, FPN height,
                         FPN devicePixelRatio);
  void (*renderCancel)(void* uptr);
  void (*renderFlush)(void* uptr);
  void (*renderFill)(void* uptr, NVGpaint* paint,
                     NVGcompositeOperationState compositeOperation,
                     NVGscissor* scissor, FPN fringe, const FPN* bounds,
                     const NVGpath* paths, int npaths);
  void (*renderStroke)(void* uptr, NVGpaint* paint,
                       NVGcompositeOperationState compositeOperation,
                       NVGscissor* scissor, FPN fringe, FPN strokeWidth,
                       const NVGpath* paths, int npaths);
