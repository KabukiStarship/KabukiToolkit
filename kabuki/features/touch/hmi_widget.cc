...
@file    /kabuki/features/hmi/hmi_widget.cc
...
Widget::Widget(const char* initLabel) : HmiComponent(initLabel), numPages(1) {
...
const char* Widget::GetName() { return WidgetName; }

bool Widget::GetName(const char* name) {  // Renames this Widget to name
...
int Widget::GetNumPages() { return numPages; }
...
int Widget::insertDuplicate(const char* name) {
...
int Widget::insertPage(
    const char* name) {  // This function inserts a new Widget page in d
...
WidgetPage* Widget::page(const char* name) {
...
int Widget::deletePage(uint32_t pageIndex) {
  uint32_t i;
...
int Widget::infoValue() { return debugValue; }
