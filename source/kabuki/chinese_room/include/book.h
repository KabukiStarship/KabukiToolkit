...
class Book: public Operation
{
    ...
    Book** root_;       //< Pointer to the dynamically allocated bags.
    Book* bag_;         //< The currently selected bag.
...
template<typename TIndex, typename TKey, typename TData, typename THash, 
    uint MaxStackSize>
KABUKI void Destruct (Book<TIndex, TKey, TData, THash, MaxStackSize>* r) {
    if (r == nullptr) return;
