...
#ifndef KABUKI_CARDS__DECK_H_INCLUDED
#define KABUKI_CARDS__DECK_H_INCLUDED
...
public:
    ...
    */
    Deck (bool deckContainsJokers = hasNoJokers, int acesAreHigh = acesHigh,  Card::SuitCulture thisCardSuitCulture = Card::french, 
        String& rearDeckImageFilename = String (defaultRearDeckImage), const String& deckArtDirectory = defaultDeckArtDirectory);
    virtual ~Deck () {}                                 //< Destructor.

    Deck& operator= (const Deck& other);                //< Operator= overlaoder deep copies the state of the other object.
    ...
    int getSuitValue (int suit);                        /*< Gets the suit value of the given suit.
                                                        @pre    suit must be between 1-4.
                                                        @return Returns a number between 1-4 upon sucess, and -1 if the index was out of bounds. */
    bool deckHasJokers ();                              //< Returns true if this deck has Jokers.

    int getNumCards ();                                 //< Returns the number of cards in the deck.
    int getAcesHigh ();                                 //< Returns the if aces are high or low.
    Card* getCard (int index);                          /*< Returns a pointer to the Card at the spcified index.
                                                            @return Returns nullptr if the index is out of bounds. */

    int setRearImage (const File& thisFile);            /*< Sets the frontImage to a new Image from thisFile.
                                                            @pre    thisFile must be a valid existing Image file.
                                                            @return returns -1 if thisFile is not a valid Image. */
    int setDeckArt (String& deckArtFolder);             /*< Loads the art from the deckArtFolder.
    ...
    String getSuitString (Card::Suit suit);             //< Returns a String representation of the suit.
...
#endif // KABUKI_CARDS__DECK_H_INCLUDED