...
#include <ChineseRoom/FileInputStream.h>
#ifdef ANDROID
#include <_/Android/ResourceStream.h>
#endif
...
bool FileInputStream::open (const std::string& filename)
{
    ...
    if (m_file)
        std::fclose (m_file);
    errno_t err;
    if (err = std::fopen_s (&m_file, filename.c_str (), "rb") != 0) 
    {
        puts ("Error opening FileInputStream: ");
        puts (strerror(err));
        puts ("\n");
        return false;
    }
...
int64_t FileInputStream::read (void* data, int64_t size)
{
    #ifdef ANDROID
...
int64_t FileInputStream::seek (int64_t position)
{
    #ifdef ANDROID
