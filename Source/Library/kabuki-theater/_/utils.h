...
INLINE void PrintDebugPointer (const char* message, const void* address) {
#if DEBUG
    printf ("| Error at address 0x%p: %s\n", address, message);
#endif
}

INLINE void PrintDebugHex (const char* message, char value) {
#if DEBUG
    printf ("| %s '%c':0x%x\n", message, value, value);
#endif
}
...
template<typename T>
INLINE void PrintDebugSignedHex (const char* message, T value) {
#if DEBUG
    printf ("| %s:'%i':0x%x\n", message, value, value);
#endif
}
...
template<typename T>
INLINE void PrintDebugUnsignedHex (const char* message, T value) {
#if DEBUG
    printf ("%s:'%u':0x%x\n", message, value, value);
#endif
}
...
template<typename T>
INLINE void PrintDebugError (const char* message, T expected, T found) {
#if DEBUG
...
*/
INLINE int StringLength (const char* s, char delimiter = 0) {
    if (s == nullptr)
        return -1;
    int count = 0;
...
INLINE const char* DuplicateString (const char* s, char delimiter = 0) {
...
INLINE void PrintMemory (const void* address, const void* end) {
    printf ("|%i", 0);
    // Print columns
...
INLINE void PrintBar (const char* input) {
    std::cout << "\n| " << input << '\n';
}

/** Prints a line break. */
INLINE void PrintBreak (const char* header = "\n_", char c = '_', int num_lines = 0, 
...
INLINE void PrintCentered (const char* input, int width, bool is_last = false,
...
INLINE void PrintColumnBreak (int num_columns, char column_delimeter = '|', char break_char = '-', 
                       int width = 80) {
...
INLINE const char* FindEndOfRow (const char* input, int num_columns) {
    char c;
...
/** Prints the given string centered with a horizontal page bar to the left 
    ...
    @param num_columns 
*/
INLINE void PrintPageCentered (const char* input, int num_columns) {
...
INLINE void PrintPageRight (const char* input, int num_columns) {
    char c;  //< The current char.
 ...
 */
INLINE void PrintPage (const char* input = "", int indentation = 0,
...
INLINE const char* NextNonNumberString (const char* input) {
    char c = *input;
...
/** Returns the pointer to the next char in the string that is not an ASCII 
    number.
    @return A pointer to the next non-number in the input string. */
INLINE char* NextNonNumber (char* input) {
    return (char*)NextNonNumberString (input);
}

/** Skips the leading zeros of a number if there are any. */
INLINE const char* SkipLeadingZerosString (const char* input) {
    if (input == nullptr)
        return nullptr;
    char c = *input,
        d;
...
/** Skips the leading zeros of a number if there are any. */
INLINE char* SkipLeadingZeros (char* input) {
    return (char*)SkipLeadingZerosString (input);
}

/** Skips all the spaces at the start of the string. */
INLINE const char* SkipSpacesString (const char* input) {
    if (input == nullptr)
        return 0;
    //std::cout << "\nSkipping spaces: ";
    char c = *input;
...
/** Skips all the spaces at the start of the string. */
INLINE char* SkipSpaces (char* input) {
...
INLINE const char* EndOfTokenString (const char* input) {
    if (input == nullptr)
        return nullptr;
...
/** Gets the end of the current token from the string.
    @return Returns a pointer to the end of the token. */
INLINE char* EndOfToken (char* input) {
    return (char*)EndOfTokenString (input);
}

/** Scrolls over to the next double quote mark. */
INLINE const char* EndOfString (const char* input, char delimiter = '\"') {
    char c = *input;
    while (c) {
        if (c == delimiter)
            return input;
...
/** Compares the source and query string using the delimiter to terminate the query. */
INLINE char* CompareToken (const char* input, const char* query) {
    return (char*)CompareTokenString (input, query);
}

/** Compares the source and query string using the delimiter to terminate the query. */
INLINE const char* CompareString (const char* input, const char* query,
                                  char delimiter = 0) {
    if (input == nullptr)
        return nullptr;
    if (query == nullptr)
...
/** Compares the source and query string using the delimiter to terminate the
    query. */
INLINE char* Compare (char* source, const char* query, char delimiter = 0) {
...
/** Parses the given string for the given token, setting the following 
    ...
    @return Returns null if the search failed and a pointer to the first hit
            upon success.
*/
INLINE const char* ParseString (const char* input, char* destination,
...
/** Parses the given string for the given token, setting the following 
    ...
    @return Returns null if the search failed and a pointer to the first hit
            upon success.
*/
INLINE char* Parse (char* input, char* destination, int buffer_size,
                    char delimiter = 0) {
...
/** Parses the token by scrolling to the end of it and setting the end to null.
    ...
    @return Returns null if the search failed and a pointer to the first hit
            upon success. */
INLINE char* ParseToken (char* input) {
    if (input == nullptr)
        return nullptr;
...
INLINE const char* FindString (const char* input, const char* query, char delimiter = 0) {
    if (input == nullptr)
        return nullptr;
    if (query == nullptr)
        return nullptr;
...
/** Searches the given string for the given string, setting the following 
    ...
    @return Returns null if the search failed and a pointer to the first hit
            upon success.
*/
INLINE char* Find(char* input, const char* query, char delimiter = 0) {
...
INLINE const char* ParseIntString (const char* input, int* value) {
    if (input == nullptr)
        return nullptr;
    if (value == nullptr)
        return nullptr;
...
/** Parses the string for a int.
    @return Returns null there was not a number and a pointer to after the 
            number ends in the string upon success. */
INLINE char* ParseInt (char* input, int* value) {
...
INLINE const char* ParseFloatString (const char* input, float* value) {
    if (input == nullptr)
        return nullptr;
    if (value == nullptr)
        return nullptr;
...
/** Parses the string for a float.
    @return Returns null there was not a number and a pointer to after the 
            number ends in the string upon success. */
INLINE char* ParseFloat (char* input, float* value) {
...
INLINE bool IsToken (const char* input) {
    if (input == nullptr)
        return false;
    char c = *input;
    if (!isspace (c))
