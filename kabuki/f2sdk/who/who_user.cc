/* Kabuki Toolkit
    @file    $kabuki-toolkit/library/kt/id/user.cc
...
User::User(Authenticator* authenticator, uid_t uid, const char* handle,
           const char* password, double balance, int64_t value)
...
const char* User::GetStatus() { return status_; }
...
uid_t User::GetUid() { return uid_; }

int32_t User::GetSession() { return session_; }
...
uid_t User::GetSessionKey() { return public_key_; }

uid_t User::GetResponse() { return response_; }
...
double User::GetBalance() { return balance_; }
...
int64_t User::GetValue() { return value_; }
...
Expression* User::GetSlot() { return slot_; }

#if SCRIPT_USING_MIRROR
Mirror* User::GetMirror() { return nullptr; }
...
const Op* User::Star(wchar_t index, Expression* expr) {
  static const Op This{"User", OperationCount(0), OperationFirst('A'),
                       "A user account.", 0};
  switch (index) {
    case '?':
      return ExpressionOperand(expr, &This);
  }
  return nullptr;
}
