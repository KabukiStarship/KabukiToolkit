...
/*
const Operation* MirrorResult (Mirror* mirror, Bin::Error error) {
    return BinResult (reinterpret_cast<Bin*> (mirror), error);
}

const Operation* MirrorResult (Mirror* mirror, Bout::Error error) {
...
const Operation* MirrorWrite (Mirror* mirror, const uint_t* params, void** args) {
    if (mirror == nullptr)     //< Is this a double check?
        return MirrorResult (mirror, Bout::kErrorRoom);
    if (params == nullptr)
        return MirrorResult (mirror, Bout::kErrorRoom);
    if (args == nullptr)
        return MirrorResult (mirror, Bout::kErrorRoom);
...
const Operation* MirrorRead (Mirror* mirror, const uint_t* params, void** args) {
    if (mirror == nullptr)
        return MirrorResult (mirror, Bin::kErrorRoom);
    if (params == nullptr)
        return MirrorResult (mirror, Bin::kErrorRoom);
    if (args == nullptr)
        return MirrorResult (mirror, Bin::kErrorRoom);
            ...
            switch (type) {
                ...
                case STR:
                    ...
                    *ui1_ptr = ui1;
                    ...
                    while (ui1 != 0 && count != 0) {
                        if (count-- == 0)
                            return MirrorResult (mirror, Bin::kErrorBufferUnderflow,
                                                 params, index, start);
                ...
                case BOL:
                    ...
                    if (length == 0)
                        return MirrorResult (mirror, Bin::kErrorBufferUnderflow, params,
                                             index, start);
                    --length;
                ...
                case DBL:
                    ...
                    Read8ByteType:{
                        ...
                        if (length < offset + 8)
                            return MirrorResult (mirror, Bin::kErrorBufferUnderflow, params, index, start);
                        length -= offset + 8;
                        start += offset;
                        if (start >= end) start -= size;    //< Bound
                                                            // Read from buffer and write to the stack:
                ...
                default:
                {
                    ...
                    if (count && (type >= OBJ)) {
                        ...
                        return MirrorResult (mirror, Bin::kErrorInvalidType, params, index, start);
                    }
                    // We don't care if it's a multidimensional array anymore.
                    ui1_ptr = reinterpret_cast<uint8_t*> (args[index]);
                    if (ui1_ptr == nullptr)
                        return MirrorResult (mirror, Bin::kErrorRoom, params, index, start);
                    count &= 0x3;
                    switch (count) {
                        case 0:
                        { // It's a 8-bit count.
                            if (type >= LST) {
                                ...
                                return MirrorResult (mirror, Bin::kErrorInvalidType,
                                                     params, index, start);
                            }
                        ...
                        { // It's a 16-bit count.
                            ...
                            if (count > length)
                                return MirrorResult (mirror, Bin::kErrorBufferOverflow,
                                                     params, index, start);
                            break;
                        }
                        case 2:
                    ...
                    if (length < count)
                        return MirrorResult (mirror, Bin::kErrorBufferOverflow, params, index, start);
                    if (count == 0)
                    ...
                    InvalidType:
                    {
                        ...
                        return MirrorResult (mirror, Bin::kErrorInvalidType, params,
                                             index, start);
                    }
                }
                //std::cout << " |";
...
const Operation* MirrorResult (Mirror* bin, Bin::Error error,
                             const uint_t* header) {
    return 0;
}

const Operation* MirrorResult (Mirror* bin, Bout::Error error,
                             const uint_t* header) {
    return 0;
}
