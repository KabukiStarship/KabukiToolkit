/** Kabuki Toolkit
    @version 0.x
    @file    ~/libraries/crabs/crabs_slot.cc
...
const Op* Slot::Read (const Op& op, void** args) {
    return Read (op.in, args);
}

const Op* Slot::Write (const Op& op, void** args) {
    return Write (op.out, args);
}
...
const Op* Slot::Write (const char* message) {
    return nullptr;
