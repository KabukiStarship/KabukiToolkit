...
const char* TextLineEnd(const char* target, int num_columns, char delimiter) {
  char c;
...
const char* TextFind(const char* target, const char* query, char delimiter) {
  ...
  if (c <= delimiter)     //< We're not allowing empty queries.
    return nullptr;
  const char *start_of_query, *cursor = target;
...
const char* TextLineEnd(const char* text, const char* text_end, int num_columns,
                        char delimiter) {
  if (!text) {
    return nullptr;
  }
