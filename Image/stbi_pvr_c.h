...
static int stbi__pvr_info(stbi__context *s, int *x, int *y, int *comp,
                          int *iscompressed) {
  ...
  *comp = s->img_n = (header.dwBitCount + 7) / 8;
  ...
  if (iscompressed) *iscompressed = 0;
  ...
  switch (header.dwpfFlags & PVRTEX_PIXELTYPE) {
    ...
    case OGL_PVRTC2:
      s->img_n = 4;
      if (iscompressed) *iscompressed = 1;
      break;
    case OGL_PVRTC4:
      s->img_n = 4;
      if (iscompressed) *iscompressed = 1;
...
int stbi__pvr_info_from_memory(stbi_uc const *buffer, int len, int *x, int *y,
                               int *comp, int *iscompressed) {
  stbi__context s;
  stbi__start_mem(&s, buffer, len);
  return stbi__pvr_info(&s, x, y, comp, iscompressed);
}

int stbi__pvr_info_from_callbacks(stbi_io_callbacks const *clbk, void *user,
...
#ifndef STBI_NO_STDIO
int stbi__pvr_info_from_path(char const *filename, int *x, int *y, int *comp,
                             int *iscompressed) {
  int res;
  FILE *f;
  errno_t err = fopen_s(&f, filename, "rb");
...
int stbi__pvr_info_from_file(FILE *f, int *x, int *y, int *comp,
                             int *iscompressed) {
  stbi__context s;
  int res;
  long n = ftell(f);
...
#define LIMIT_COORD(Val, Size, AssumeImageTiles)  \
  ((AssumeImageTiles) ? WRAP_COORD((Val), (Size)) \
                      : PVRT_CLAMP((Val), 0, (Size)-1))
...
/*!***********************************************************************
 ...
 @Returns		TRUE if the number is an integer power of two, else
FALSE.
...
/*!***********************************************************************
 ...
 @Description	Given the block and the texture type and it's relative
                                position in the 2x2 group of blocks, extract the
bit patterns for the fully defined pixels.
*************************************************************************/
static void UnpackModulations(const AMTC_BLOCK_STRUCT *pBlock,
  ...
  } else if (Do2bitMode)  // else if direct encoded 2bit mode - i.e. 1 mode bit
                          // per pixel
  {
    for (y = 0; y < BLK_Y_SIZE; y++) {
      for (x = 0; x < BLK_X_2BPP; x++) {
...
/*!***********************************************************************
 ...
 @Description	This performs a HW bit accurate interpolation of either the
                                ...
                                NOTE: It is assumed that the source colours are
in ARGB 5554 format - This means that some "preparation" of the values will be
necessary.
*************************************************************************/
...
static void GetModulationValue(int x, int y, const int Do2bitMode,
  ...
  *DoPT = 0;
  ...
  if (ModulationModes[y][x] == 0) {
    ...
    if (((x ^ y) & 1) == 0)
      ModVal = RepVals0[ModulationVals[y][x]];
    else if (ModulationModes[y][x] ==
             1)  // else average from the neighbours if H&V interpolation..
    {
...
/*!***********************************************************************
 ...
 @Description	Given the Block (or pixel) coordinates and the dimension of
                                the texture in blocks (or pixels) this returns
the twiddled offset of the block (or pixel) from the start of the map.

                                NOTE the dimensions of the texture must be a
power of 2
*************************************************************************/
...
static U32 TwiddleUV(U32 YSize, U32 XSize, U32 YPos, U32 XPos) {
  ...
  if (DisableTwiddlingRoutine) return (YPos * XSize + XPos);
...
static void Decompress(AMTC_BLOCK_STRUCT *pCompressedData, const int Do2bitMode,
  ...
  for (y = 0; y < YDim; y++) {
    for (x = 0; x < XDim; x++) {
      ...
      */
      ...
      pBlocks[0][1] =
          pCompressedData + TwiddleUV(BlkYDim, BlkXDim, BlkY, BlkXp1);
      pBlocks[1][0] =
          pCompressedData + TwiddleUV(BlkYDim, BlkXDim, BlkYp1, BlkX);
      pBlocks[1][1] =
          pCompressedData + TwiddleUV(BlkYDim, BlkXDim, BlkYp1, BlkXp1);

      /*
      // extract the colours and the modulation information IF the previous
      values
      // have changed.
      */
      if (memcmp(pPrevious, pBlocks, 4 * sizeof(void *)) != 0) {
        ...
        for (i = 0; i < 2; i++) {
          ...
          for (j = 0; j < 2; j++) {
            ...
            UnpackModulations(pBlocks[i][j], Do2bitMode, ModulationVals,
                              ModulationModes, StartX, StartY);
      ...
      */
      InterpolateColours(Colours5554[0][0].Reps[0], Colours5554[0][1].Reps[0],
                         Colours5554[1][0].Reps[0], Colours5554[1][1].Reps[0],
                         Do2bitMode, x, y, ASig);

      InterpolateColours(Colours5554[0][0].Reps[1], Colours5554[0][1].Reps[1],
                         Colours5554[1][0].Reps[1], Colours5554[1][1].Reps[1],
                         Do2bitMode, x, y, BSig);

      GetModulationValue(x, y, Do2bitMode, (const int(*)[16])ModulationVals,
                         (const int(*)[16])ModulationModes, &Mod, &DoPT);
...
static void *stbi__pvr_load(stbi__context *s, int *x, int *y, int *comp,
                            int req_comp) {
  ...
  *comp = s->img_n;
  ...
  if (iscompressed) {
    ...
    Decompress((AMTC_BLOCK_STRUCT *)pvr_data, bitmode, s->img_x, s->img_y, 1,
               (unsigned char *)pvr_res_data);
    free(pvr_data);
  } else {
  ...
  if ((req_comp <= 4) && (req_comp >= 1)) {
    ...
    if (req_comp != s->img_n) {
      pvr_res_data = stbi__convert_format(pvr_res_data, s->img_n, req_comp,
                                          s->img_x, s->img_y);
      *comp = req_comp;
    }
  }
...
#ifndef STBI_NO_STDIO
void *stbi__pvr_load_from_file(FILE *f, int *x, int *y, int *comp,
                               int req_comp) {
  stbi__context s;
  stbi__start_file(&s, f);
...
void *stbi__pvr_load_from_path(char const *filename, int *x, int *y, int *comp,
                               int req_comp) {
  void *data;
  FILE *f;
  errno_t err = fopen_s(&f, filename, "rb");
...
void *stbi__pvr_load_from_memory(stbi_uc const *buffer, int len, int *x, int *y,
...
void *stbi__pvr_load_from_callbacks(stbi_io_callbacks const *clbk, void *user,
