...
@file    /kabuki/features/touch/touch_mixer_page.cc
...
MixerPage::MixerPage(Mixer* m, SI4 initStartIndex) : mixer(m) {
  ...
  for (SI4 i = 0; i < 8; ++i) channel[i] = 0;
}

MixerPage::~MixerPage() {
  for (SI4 i = 0; i < 8; ++i) delete channel[i];
  delete channel;
  delete mixer;
}

SI4 MixerPage::startIndex() { return startingIndex; }

void MixerPage::SetStartIndex(SI4 index) {
  if (index < 0) return;
  if (mixer != 0)
...
const CH1* MixerPage::SetState(byte Value) { return 0; }

const CH1* MixerPage::op(Expression* io, byte index) {
  switch (Index) {
    case '?':
