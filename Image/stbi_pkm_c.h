...
int stbi__pkm_info_from_memory(stbi_uc const *buffer, int len, int *x, int *y,
                               int *comp) {
  stbi__context s;
  stbi__start_mem(&s, buffer, len);
...
int stbi__pkm_info_from_callbacks(stbi_io_callbacks const *clbk, void *user,
...
static void *stbi__pkm_load(stbi__context *s, int *x, int *y, int *comp,
                            int req_comp) {
  ...
  *comp = s->img_n = 3;
  ...
  res = etc1_decode_image((const etc1_byte *)pkm_data,
                          (etc1_byte *)pkm_res_data, width, height, 3, bpr);
  ...
  if (0 == res) {
    if ((req_comp <= 4) && (req_comp >= 1)) {
      ...
      if (req_comp != s->img_n) {
        pkm_res_data = stbi__convert_format(pkm_res_data, s->img_n, req_comp,
                                            s->img_x, s->img_y);
        *comp = req_comp;
      }
    }
...
#ifndef STBI_NO_STDIO
void *stbi__pkm_load_from_file(FILE *f, int *x, int *y, int *comp,
                               int req_comp) {
  stbi__context s;
  stbi__start_file(&s, f);
...
void *stbi__pkm_load_from_path(char const *filename, int *x, int *y, int *comp,
                               int req_comp) {
  void *data;
  FILE *f;
  errno_t err = fopen_s(&f, filename, "rb");
...
void *stbi__pkm_load_from_memory(stbi_uc const *buffer, int len, int *x, int *y,
...
void *stbi__pkm_load_from_callbacks(stbi_io_callbacks const *clbk, void *user,
