...
#include <graphics/nanovg_gl_utils.h>
#include "perf.h"

void renderPattern(NVGcontext* vg, NVGLUframebuffer* fb, FPW t, FPW pxRatio) {
  int winWidth, winHeight;
  int fboWidth, fboHeight;
  int pw, ph, x, y;
  FPW s = 20.0f;
  FPW sr = (cosf(t) + 1) * 0.5f;
  FPW r = s * 0.6f * (0.2f + 0.8f * sr);
...
int main() {
  ...
  pxRatio = (FPW)fbWidth / (FPW)winWidth;
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (fb != NULL) {
      ...
      nvgBeginPath(vg);
      nvgRoundedRect(vg, 140 + sinf(t * 1.3f) * 100,
                     140 + cosf(t * 1.71244f) * 100, 250, 250, 20);
      nvgFillPaint(vg, img);
      nvgFill(vg);
    ...
    for (i = 0; i < n; i++) updateGraph(&gpuGraph, gpuTimes[i]);
