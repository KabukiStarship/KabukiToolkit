...
INLINE Wall* WallInit (uint_t size) {
...
INLINE Door** GetDoors (Wall* wall) {
...
/** Adds a Door to the slot.
    @return Returns nullptr if the Door is full and a pointer to the Door in the 
            buffer upon success. */
INLINE Door* AddDoor (Wall* wall, Door* t) {
...
INLINE Door* GetDoor (Wall* wall, byte index) {
    if (s == nullptr) return nullptr;
...
INLINE void Delete (Wall* wall, byte index) {
    if (s == nullptr) return;
    if (index >= d->num_doors)
        return;
    Door** ts = GetDoors (s);
