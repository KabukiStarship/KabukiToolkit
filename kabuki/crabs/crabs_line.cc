...
Printer1& Line::Print(Printer1& print) {
  char* begin = print.begin;
  ASSERT(begin);
  char* end = print.end;
  int l_column_count = column_count;
  if (begin + l_column_count + 1 >= end) return print;

  *begin++ = '\n';

  char l_token = token;
  while (l_column_count-- > 0) *begin++ = l_token;
...
Printer1& StringLine::Print(Printer1& print) {
  char* begin = print.begin;
  ASSERT(begin);
  char* end = print.end;
  int l_column_count = column_count;
