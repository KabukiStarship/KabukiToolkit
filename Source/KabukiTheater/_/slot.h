...
static Slot* SlotInit (uint_t slot_size) {
...
/** Initializes a Slot at the beginning of the given buffer. */
static Slot* SlotInit (byte* buffer, uint_t slot_size) {
...
/** Adds a Terminal to the slot.
    @return Returns nullptr if the Terminal is full and a pointer to the Slot in the 
            buffer upon success. */
static ticket_t SlotAddTerminal (Slot* s, Terminal* t) {
...
static Terminal* SlotGetTerminal (Slot* s, byte index) {
    if (s == nullptr) return nullptr;
...
/** Gets the Terminal from the Slot at the given index. */
static Terminal* SlotFindTerminal (Slot* s, void* address) {
...
static void SlotDelete (Slot* s, byte index) {
    if (s == nullptr) return;
    if (index >= s->num_slots)
        return;
