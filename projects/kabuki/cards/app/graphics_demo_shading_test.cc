...
{
    final SI4 WIDTH = 640, HEIGHT = 480, halfWIDTH = 320, halfHEIGHT = 240;
    SI4 clipMinX = 0, clipMinY = 0, clipMaxX = WIDTH, clipMaxY = HEIGHT, clipNearZ = 0, clipFarZ = 640;
    FLT[] cos, sin;
    ...
    {  viewingDistance = 250;
        ...
        lightSource = new d3.Vector_d ((FLT)-.913913,(FLT).389759,(FLT)-.113369);
    ...
    {  page.fillRect (0,0,WIDTH,HEIGHT);
        for (SI4 i = 0; i<testModel.numVertices; i++)
    ...
    {  BOL polygonIsQuad = false;
        ...
        for (SI4 i = 0; i<renderShape.cameraVertices.length; i++)
        ...
        for (SI4 currPoly = 0; currPoly<renderShape.polygon.length; currPoly++)
    ...
    {  if ((one.x! = two.x&& two.x! = three.x)||(one.y! = two.y&& two.y! = three.y))
        {  d3.Point_d temp;            
            ...
            {  FLT newX = one.x + ((two.y-one.y)*(three.x-one.x)/(three.y-one.y));
    ...
    {  FLT deltaXright, deltaXleft, xStart, xEnd, height, right, left;
        ...
        {  for (SI4 y = (SI4)one.y; y< = (SI4)three.y; y++)
            {  page.drawLine ((SI4)xStart, y, (SI4)xEnd, y);
                xStart+ = deltaXleft;
                xEnd  + = deltaXright;
            }
        }
        ...
        {  for (SI4 y = (SI4)one.y; y< = (SI4)three.y; y++)
            {  left = xStart;
                ...
                if (right>clipMaxX)
                    right =  clipMaxX;
                page.drawLine ((SI4)left, y, (SI4)right, y);
            }
    ...
    {  FLT deltaXright, deltaXleft, xStart, xEnd, height, right, left;
        ...
        {  for (SI4 y = (SI4)one.y; y< = three.y; y++)
            {  left = xStart;
                right = xEnd;
                xStart + =  deltaXleft;
                xEnd    + =  deltaXright;
    ...
    {  cos = sin = new FLT[360];
        for (SI4 i = 0; i<360; i++)
        {  cos[i] = (FLT)Math.toDegrees(Math.cos(i));
            sin[i] = (FLT)Math.toDegrees(Math.sin(i));
        }
    }
    FLT sin (SI4 degree)
