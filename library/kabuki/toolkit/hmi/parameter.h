...
#ifndef HEADER_FOR_KABUKI_TOOLKIT_HMI_PARAMETER
#define HEADER_FOR_KABUKI_TOOLKIT_HMI_PARAMETER
...
class KABUKI Parameter : public HmiComponent {
    ...
    Parameter (uint16_t newType, const char* label = "", int channel = 0, 
               uint16_t init_value = 0, uint16_t min_value = 0, uint16_t max_value_ = 1);
...
class ParameterPair {
    ...
    ParameterPair (uint16_t& msb, uint16_t& lsb) :
        msb(msb),
        lsb(lsb)
    {
...
#endif  //< HEADER_FOR_KABUKI_TOOLKIT_HMI_PARAMETER
