...
@file    ~/kabuki/crabs/crabs_types.cc
...
#else
  if (type <= BOL) return AlignUpPointer2<>(value);
#endif
  if (type <= TMS) return AlignUpPointer4<>(value);
  if (type <= DEC) return AlignUpPointer8<>(value);
