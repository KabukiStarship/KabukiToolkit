...
//import java.awt.geom.PathIterator_f;
//import java.util.Vector;
//import java.util.Enumeration;

#include <_Math/Calc.hpp>
#include <_Math/Crossings.hpp>

class Crossings;

Crossings* findCrossings (vector<Curve_f*>& curves,  float xlo, float ylo, float xhi, float yhi);

Crossings* findCrossings (PathIterator_f pi, float xlo, float ylo, float xhi, float yhi);
...
class Crossings 
{
    ...
    Crossings (float xlo, float ylo, float xhi, float yhi) 
    {
        xlo = xlo;
        ylo = ylo;
        xhi = xhi;
    ...
    bool accumulateLine (float x0, float y0, float x1, float y1)
    {
        if (y0 <= y1)
    ...
    bool accumulateLine (float x0, float y0, float x1, float y1, int direction)
    {
        if (yhi <= y0 || ylo >= y1)
            return false;
        if (x0 >= xhi && x1 >= xhi)
    ...
    bool accumulateQuad (float x0, float y0, float* coords) 
    {
        ...
        Curve_f.insertQuad (tmp, x0, y0, coords);
        Enumeration enum_ = tmp.elements ();
        while (enum_.hasMoreElements ())
        {
            Curve_f c = (Curve_f) enum_.nextElement ();
    ...
    bool accumulateCubic (float x0, float y0, float* coords) 
    {
        ...
        Curve_f.insertCubic (tmp, x0, y0, coords);
        Enumeration enum_ = tmp.elements ();
    ...
    virtual void record (float ystart, float yend, int direction) = 0;
    ...
    inline void print (Terminal& io) 
    {
        ...
        index_t l = 13;
        static const index_t header[] { 5, STR, l, FLT, STR, l, FLT, STR, l };
        slot.print (header, "  bounds = [", ylo, ", ",yhi, "]");
        slot.print (header, "  [", yranges[i], ", ", yranges[i + 1], "]");
        slot.prints ("]\n");
    }
...
class EvenOddCrossings: public Crossings 
{
    EvenOddCrossings (float xlo, float ylo, float xhi, float yhi)
    :   Crossings (xlo, ylo, xhi, yhi)
    {
    }

    bool covers (float ystart, float yend) 
    {
        return (limit == 2 && yranges[0] <= ystart && yranges[1] >= yend);
    }

    void record (float ystart, float yend, int direction)
    {
        if (ystart >= yend)
            return;
...
class NonZeroCrossings: public Crossings 
{
    public:
    
    NonZeroCrossings (float xlo, float ylo, float xhi, float yhi) 
    :   Crossings (xlo, ylo, xhi, yhi)
    {
    ...
    void insert (int cur, float lo, float hi, int dir) 
    {
        int rem = limit - cur;
        float* oldranges = yranges;
        int* oldcounts = crosscounts;

        if (limit >= yranges.length) 
        {
            yranges = new float[limit + 10];
    ...
    void record (float ystart, float yend, int direction) 
    {
        ...
        if (cur < limit)
        {
            ...
            float yrlo = yranges[cur + 0];
            float yrhi = yranges[cur + 1];
...
Crossings* findCrossings (vector<Curve_f*>& curves,  float xlo, float ylo, float xhi, float yhi)
{
    Crossings* cross = new EvenOddCrossings (xlo, ylo, xhi, yhi);

    for_each (curves.begin (), curves.end (), [](Curve_f& c, Crossings* cross) 
    {
...
Crossings* findCrossings (PathIterator_f pi, float xlo, float ylo, float xhi, float yhi)
{
    ...
    if (pi.getWindingRule () == pi.WIND_EVEN_ODD)
        cross = new EvenOddCrossings (xlo, ylo, xhi, yhi);
    else
        cross = new NonZeroCrossings (xlo, ylo, xhi, yhi);
    ...
    while (!pi.isDone ())
    {
        ...
        switch (type)
        {
            case PathIterator_f.SEG_MOVETO:
                if (movy != cury &&
                    cross.accumulateLine (curx, cury, movx, movy)) return nullptr;
            ...
            case PathIterator_f.SEG_LINETO:
                newx = coords[0];
                newy = coords[1];
                if (cross.accumulateLine (curx, cury, newx, newy)) return nullptr;
            ...
            case PathIterator_f.SEG_QUADTO:
                ...
                if (cross.accumulateQuad (curx, cury, coords)) return nullptr;
            ...
            case PathIterator_f.SEG_CUBICTO:
                newx = coords[4];
                newy = coords[5];
                if (cross.accumulateCubic (curx, cury, coords)) return nullptr;
