...
@file    /kabuki/features/touch/touch_isymmetric_controller.cc
...
IsymmetricController::IsymmetricController()
  ...
  for (SI4 i = 0; i < MaxTemplates; ++i) templatesArray[i] = nullptr;
}
...
void IsymmetricController::pressModeButton(ParameterSet *thisLayer,
                                           Mode thisMode) {
...
void IsymmetricController::depressModeButton(ParameterSet *thisLayer,
                                             Mode thisMode) {
  // this function switches thisLayer's current mode

  Tms currentTime = time(nullptr), doubleClickVector;

  doubleClickVector = currentTime - thisLayer->LastModePress();

  if (doubleClickVector >
      _doubleClickTime)  // then swap the layer controls back
  {
    holdSwap();
  } else
  ...
  {//
      if ((time  (nullptr) - thisLayer->LastModePress ()) <= _doubleClickTime)
  // Then its a double-click
      {// and the swap sticks
...
void IsymmetricController::holdSwap() {
  if (!swappedFlippedState) return;
}

SI4 IsymmetricController::compare(const Device &that) {
  // First, find template with same name
  String thatsName = that.label();

  for (SI4 i = 0; i < numDevices; ++i) {
...
Device *IsymmetricController::GetTemplates(SI4 index) {
  if (index < 0) return templatesArray[0];
  if (index > numDevices) return templatesArray[numDevices - 1];
  return templatesArray[index - 1];
}

Device *IsymmetricController::findDevice(CH1 *device) {
  for (SI4 i = 0; i < numDevices; ++i)
...
BOL IsymmetricController::containsDevice(const CH1 thisName[]) {
  for (SI4 i = 0; i < numDevices; ++i)
...
SI4 IsymmetricController::GetNumDevices() { return numDevices; }

BOL IsymmetricController::AddDevice(Device *newDevice) {
  // Check precondiditions
  if (newDevice == nullptr || numDevices == MaxTemplates) return false;

  CH1 *newDeviceName = newDevice->getLabel()->getText();
...
SI4 IsymmetricController::GetCommonChannel() { return commonChannel; }

SI4 IsymmetricController::GetKeyboardChannel() { return keyboardChannel; }

void IsymmetricController::SetKeyboardChannel(SI4 value) {
  if (value < 1)
    keyboardChannel = 1;
  else if (value >= 16)
    keyboardChannel = 16;
...
SI4 IsymmetricController::GetDrumsChannel() { return drumsChannel; }
void IsymmetricController::SetDrumsChannel(SI4 value) {
  if (value < 1)
    drumsChannel = 1;
  else if (value >= 16)
    drumsChannel = 16;
...
void IsymmetricController::SetTicksPerBeat(SI4 newNumTicksPerBeat) {
...
SI4 IsymmetricController::GetTicksPerBeat() { return numTicksPerBeat; }

void IsymmetricController::SetTempo(double newTempo) {
  if (theTempo < 0 || theTempo > MaxTempo) return;
...
const Op *IsymmetricController::op(_::Expression *io, byte index) {
  switch (index) {
    case '?':
      return ::_::deviceHeader<"IsymmetricController", 0, "_HMI">();
  }
