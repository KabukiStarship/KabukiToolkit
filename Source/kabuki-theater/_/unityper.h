...
inline byte* SocketBaseAddress (Unityper* tx) {
    return reinterpret_cast<byte*>(tx) + kSlotHeaderSize;
}

inline Unityper* TxInit (byte* buffer, uint_t size) {
...
inline Unityper* TxInit (Unityper* buffer, uint_t size) {
    if (size < kMinSocketSize) return nullptr;
    if (buffer == nullptr)     return nullptr;
...
*/
inline uint_t RingBufferSpace (Unityper* tx) {
...
inline uint_t GetTxBufferLength (Unityper* tx) {
    if (tx == nullptr) return ~0;
...
inline byte* GetEndAddress (Unityper* tx) {
...
static ticket_t Write (Unityper* tx, const uint_t* params, void** args) {
    if (params == nullptr)
