...
@file    /projects/kabuki_cards/source/blackjack/deck_.cc
...
Deck::Deck(BOL deck_contains_jokers, SIN aces_are_high,
           SuitCulture suit_culture, const CHR* read_deck_image_filename,
...
void Deck::Initialize(const CHR* rear_deck_image_filename,
  ...
  for (SIN suit = 1; suit <= 4; ++suit) {
    deck_.Add (Card (1, 1, ace_value, suit_values_[suit], (Card::CardSuit) suit, 
  ...
  for (SIN suit = 1; suit <= 4; ++suit) {
    ...
    for (SIN cardValue = 2; cardValue <= 13; ++cardValue) {
      ...
      // The (Card::CardSuit) "casts" the suit_value to a Card::CardSuit.
      deck_.Add(new Card(cardValue, (Card::CardSuit)suit, this));
    }
  }
