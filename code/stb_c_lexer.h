...
//     0.06 fix missing next character after ending quote mark (Andreas
//     Fredriksson) 0.05 refixed get_location because github version had lost
//     the fix 0.04 fix octal parsing bug 0.03 added
//     STB_C_LEX_DISCARD_PREPROCESSOR option
...
//     - haven't tested the FPC parsing path
...
#define STB_C_LEX_PARSE_SUFFIXES \
  N  // letters after numbers are parsed as part of those numbers, and must be
              ...
              STB_C_LEX_C_COMPARISONS(CLEX_lesseq) STB_C_LEX_C_COMPARISONS(
                  ...
                  STB_C_LEX_C_LOGICAL(CLEX_oror) STB_C_LEX_C_SHIFTS(CLEX_shl)
                      STB_C_LEX_C_SHIFTS(CLEX_shr) STB_C_LEX_C_INCREMENTS(
                          CLEX_plusplus) STB_C_LEX_C_INCREMENTS(CLEX_minusminus)
                          STB_C_LEX_C_ARITHEQ(CLEX_pluseq) STB_C_LEX_C_ARITHEQ(
                              CLEX_minuseq) STB_C_LEX_C_ARITHEQ(CLEX_muleq)
...
static double stb__clex_pow(double base, UIN exponent) {
  double value = 1;
