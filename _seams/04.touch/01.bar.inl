...
inline const CHA* Bar(CHA* seam_log, CHA* seam_end, const CHA* args) {
  ...
  D_COUT("Testing const CHA* print (_::Expr& slot)...");
  a.Print();
  ...
  D_COUT("Testing const CHA* ToString ()\n");

  w.Print();

  D_COUT("Testing InsertPage  (const CHA*&)\n");

  const CHA* page_name_String;
  CHA pageIndex = '1';
  ...
  D_COUT("Testing contains  (const CHA*&)...");

  CHECK(!w.contains("Page 1"), "Error: w does not contains \"Page 1\"");

  D_COUT("Testing DeletePage  (ISC)... ");
  ...
  D_COUT("Testing const CHA* print (_::verifier&)...");
  D_COUT(macroA.Print(_::Expr & slot));
  D_COUT("Testing add  (Parameter<ISC>*)");
  ...
  D_COUT("Added " + const CHA*(macroA.numControls()) + " test controls.\n" +
         macroA.Print(_::Expr & slot));
  D_COUT("Testing void trigger ()");
  macroA.trigger();
  D_COUT("Testing Parameter<ISC>* remove  (index);");

  D_COUT("\n\n_Theater::ProjectTests completed.\n\n");

  D_COUT("_::Label Class.\n");

  const CHA *invalid_String = "Invalid label ~",
  ...
  D_COUT("Testing ISC IsValid  (const CHA* &)");
  ...
  D_COUT("Testing ISC compare  (const CHA* &)\n");
  invalid_label.Print();
  ...
  control_b.Print();
  D_COUT("Testing ISC Compare  (const Control&)");

  CHECK(control_a.Compare(control_b) == 0);
  ISC compare_value = control_a.Compare(control_b);
  D_COUT("Testing ISC Compare  (const Control&): " + compare_value);
