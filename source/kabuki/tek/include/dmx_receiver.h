/** Kabuki Tek
    @file    /.../Source/_tek/_tek/DMX/DMXReceiver.h
...
/** DMXReceiver receiver.
    @param NumChannels  The number of DMX channels  (must be less than 512).
    @param NumChannels  The start DMX channel  (must be less than 512).
 */
...
{  
    ...
    void SetChannel  (uint16_t Index, byte Value)
    {
        if (Index >= kNumDMXChannels || 
            Index <  StartChannel || 
            Index >  StartChannel + NumChannels - 1)
        {
            return;
        }
    ...
    {
        if (index >= StartChannel + NumChannels) return 0;
        return data[index];
    }
    ...
    /** Marks the DMX data as being recieved. */
    int MarkReceived ()
    {
        int r = dataReceived;
        dataReceived = 0;
...
#endif KABUKI_TEK_DMX_DMXRECIEVER_H
