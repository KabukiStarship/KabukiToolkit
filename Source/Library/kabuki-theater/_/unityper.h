...
INLINE byte* UnityperSlot (Unityper* tx) {
    return reinterpret_cast<byte*>(tx) + kSlotHeaderSize;
}

INLINE Unityper* UnityperInit (byte* buffer, uint_t size) {
...
INLINE Unityper* UnityperInit (Unityper* buffer, uint_t size) {
    if (size < kMinSocketSize) return nullptr;
    if (buffer == nullptr)     return nullptr;
...
*/
INLINE uint_t UnityperSpace (Unityper* tx) {
...
INLINE uint_t UnityperTxBufferLength (Unityper* tx) {
    if (tx == nullptr) return ~0;
...
INLINE byte* UnityperEndAddress (Unityper* tx) {
...
INLINE byte UnityperStreamByte (Unityper* tx) {
...
*/
INLINE ticket_t Read (Unityper* rx, const uint_t* params, void** args) {
    if (rx == nullptr)
