 ...
 * are in :ref:`file_nanogui_serializer_core.h`, see :class:`_::Serializer`.
 * \endrst
 */
...
public:
  ...
  enum {
    kHeaderIDLength = 6,
    kHeaderSize =
    kHeaderIDLength + sizeof (uint64_t) + sizeof (uint32_t),
  };
  ...
  template <typename T> void set (const std::string &name, const T &value) {
    typedef serialization_helper<T> helper;
...
protected:
  ...
  const char* HeaderID () { return "SER_V1"; }
  ...
  void writeTOC () {
    ...
    seek (0);
    write (serialized_header_id, kHeaderIDLength);
  ...
  void readTOC () {
    ...
    uint32_t nItems = 0;
    char header[kHeaderIDLength];

    read (header, kHeaderIDLength);
    if (memcmp (header, serialized_header_id, kHeaderIDLength) != 0)
...
template <> struct serialization_helper<_::Color>
