...
import java.awt.geom.QuadraticCurve;
import java.awt.geom.CubicCurve2D;
import java.awt.geom.PathIterator_f;
import java.awt.geom.IllegalPathStateException;

static void insertMove(vector<Curve_f*> curves, float x, float y)
{
    curves->push_back (new Order0(x, y));
}

static void insertLine(vector<Curve_f*> curves, float x0, float y0, float x1, float y1)
{
    if (y0 < y1)
...
static void insertQuad(vector<Curve_f*> curves, float x0, float y0, float coords[])
{
    float y1 = coords[3];
    if (y0 > y1)
    {
...
static void insertCubic(vector<Curve_f*> curves, float x0, float y0, float* coords)
{
    float y1 = coords[5];
...
*/
static int pointCrossingsForPath(PathIterator_f pi, float px, float py)
{
...
*/
static int pointCrossingsForLine(float px, float py, float x0, float y0, float x1, float y1)
{
...
* -1 is added for each crossing where the Y coordinate is decreasing
*/
static int pointCrossingsForQuad(float px, float py, float x0, float y0, float xc, float yc, float x1, float y1, 
    int level)
{
...
* -1 is added for each crossing where the Y coordinate is decreasing
*/
static int pointCrossingsForCubic(float px, float py, float x0, float y0, float xc0, 
...
*/
static int rectCrossingsForPath(PathIterator_f pi, float rxmin, float rymin, float rxmax, float rymax)
{
...
* for the RECT_INTERSECTS constant for more complete details.
*/
static int rectCrossingsForLine(int crossings, float rxmin, float rymin, float rxmax, float rymax,
...
class Curve_f 
{
    ...
    static int orderof(float x1, float x2) 
    {
        if (x1 < x2) {
            return -1;
        }
