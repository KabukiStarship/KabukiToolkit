...
static Wall* WallInit (uint_t size) {
...
/** Adds a Door to the slot.
    @return Returns nullptr if the Door is full and a pointer to the Door in the 
            buffer upon success. */
static Door* AddDoor (Wall* wall, Door* t) {
...
static Door* GetDoor (Wall* wall, byte index) {
    if (s == nullptr) return nullptr;
...
static void Delete (Wall* wall, byte index) {
    if (s == nullptr) return;
    if (index >= d->num_doors)
        return;
    Door** ts = GetDoors (s);
