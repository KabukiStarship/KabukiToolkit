...
KABUKI byte TableAdd (Table* table, const char* key) {
...
#if DEBUG_SCRIPT_TABLE
    Text<> out;
    Write (out.Line () << "Adding Key " << key << ":" << key_length << " \n%20s: 0x%p\n%20s: %p\n%20s: 0x%p\n"
          "%20s: %p\n%20s:%u\n", , , "hashes", hashes, "key_offsets",
    ...
    if (key_length >= value) {
    #if DEBUG_SCRIPT_TABLE
        Write ("Not enough room in buffer!\n");
    #endif
        ...
        else    // Duplicate hash detected.
        {
            ...
            if (index != kInvalidIndex) { //< There are other collisions.
                ...
                while (index < kInvalidIndex) {
                    ...
                    #if DEBUG_SCRIPT_TABLE
                        Write (out << "but table already contains key at "
                              "offset:" << index);
                    #endif
                        return index;
                    }
            ...
            if (!SlotEquals (key, keys - key_offsets[index])) {
                ...
                SlotWrite (keys - value, key);
            #if DEBUG_SCRIPT_TABLE
                Write (out << "Inserting value: " << value << " into index:" <<
...
#if DEBUG_SCRIPT_TABLE
    Write ("\nThe hash " << out.Hex (hash) << " was not in the table so inserting "
          << key << "into mid:" << mid <<
          " at index " << destination - reinterpret_cast<char*> (table) <<
          " before hash " << out.Hex (hashes[mid]));
#endif
    ...
    while (low <= high) {
        ...
        if (current_hash > hash) {
            ...
            Write (out << "\nmid:" << mid << "-" << hashes[mid] << 
                  ...
                  out.Hex (Hash16 (keys - key_offsets[index])));
            #endif
