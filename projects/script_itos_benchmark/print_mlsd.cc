...
char* PrintMsld (uint32_t value, char* text, char* text_end) {
    ...
    static const char kMsbShift[] = { 4, 7, 10, 14, 17, 20, 24, 27, 30, };
    ...
    *(text + 21) = 0;
    ...
    if (value >= comparator) {
        ...
        PRINTF ("\n    Range:[%u, %u] length:%u offset:%u",
                comparator, (1 << (kMsbShift[length - 1])) - 1, (uint)length, 
                offset)
        value -= comparator;
        *text++ = '1';
        PRINT_PRINTED
        ...
        PRINTF ("\n    Range:[%u, %u] length:%u comparator:%u offset:%u", 
                1 << (kMsbShift[length - 2]), comparator, 
                (uint)length, comparator, offset)
    ...
    *text16-- = kDigits00To99[digits];
    ...
    BeginConversion:
    digits = value % 100;
    PRINTF ("\n    [%c] comparator:%u offset:%u", index + '0',
            comparator, offset)
    while (comparator < value) {
        comparator += offset;
        ++index;
    ...
    *text16-- = kDigits00To99[digits];
    ...
    power_of_ten -= 2;
    offset = *power_of_ten--;
    PRINTF ("\n    After value /= 100 offset is now %u and length is %u", offset, (uint) length)
    if (value < offset) {
