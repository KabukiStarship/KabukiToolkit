...
#define SOIL_PLATFORM_ANDROID
       ...
       defined(__DragonFly__) || defined(__SVR4))
#define SOIL_X11_PLATFORM
#endif

#if (defined(SOIL_PLATFORM_IOS) || defined(SOIL_PLATFORM_ANDROID)) && \
    (!defined(SOIL_GLES1) && !defined(SOIL_GLES2))
#define SOIL_GLES2
#endif

#if (defined(SOIL_GLES2) || defined(SOIL_GLES1)) && !defined(SOIL_NO_EGL) && \
    !defined(SOIL_PLATFORM_IOS)
#include <EGL/egl.h>
#endif
...
#if defined(SOIL_X11_PLATFORM) || defined(SOIL_PLATFORM_WIN32) || \
    defined(SOIL_PLATFORM_OSX)
...
void *SOIL_GL_GetProcAddress(const char *proc) {
  ...
  if (!soilTestWinProcPointer((const PROC)func)) func = NULL;
...
int SOIL_GL_ExtensionSupported(const char *extension) {
  ...
  if (isAtLeastGL3()) {
    ...
    if (NULL == soilGlGetStringiFunc) {
      soilGlGetStringiFunc =
          (P_SOIL_glGetStringiFunc)SOIL_GL_GetProcAddress("glGetStringi");
...
#define GL_NUM_EXTENSIONS 0x821D
    ...
    for (i = 0; i < num_exts; i++) {
      const char *thisext =
          (const char *)soilGlGetStringiFunc(GL_EXTENSIONS, i);
...
unsigned int SOIL_load_OGL_texture_from_memory(
  ...
  if (flags & SOIL_FLAG_PVR_LOAD_DIRECT) {
    tex_id = SOIL_direct_load_PVR_from_memory(buffer, buffer_length,
                                              reuse_texture_ID, flags, 0);
    if (tex_id) {
      /*	hey, it worked!!	*/
      return tex_id;
  ...
  if (flags & SOIL_FLAG_ETC1_LOAD_DIRECT) {
    tex_id = SOIL_direct_load_ETC1_from_memory(buffer, buffer_length,
...
static void createMipmaps(const unsigned char *const img, int width, int height,
                          int channels, unsigned int flags,
                          unsigned int opengl_texture_target,
                          unsigned int internal_texture_format,
                          unsigned int original_texture_format, int DXT_mode) {
...
unsigned int SOIL_internal_create_OGL_texture(
  ...
  int sRGB_texture = query_sRGB_capability() == SOIL_CAPABILITY_PRESENT &&
  ...
  needCopy =
      ((flags & SOIL_FLAG_INVERT_Y) || (flags & SOIL_FLAG_NTSC_SAFE_RGB) ||
       (flags & SOIL_FLAG_MULTIPLY_ALPHA) || (flags & SOIL_FLAG_CoCg_Y));
  ...
  if (((flags & SOIL_FLAG_POWER_OF_TWO) &&
       (!SOIL_IS_POW2(iwidth) ||
        !SOIL_IS_POW2(iheight))) || /*	user asked for it and the texture is not
                                       power of 2	*/
      ((flags & SOIL_FLAG_MIPMAPS) &&
       !((flags & SOIL_FLAG_GL_MIPMAPS) &&
         query_gen_mipmap_capability() == SOIL_CAPABILITY_PRESENT &&
         query_NPOT_capability() ==
             SOIL_CAPABILITY_PRESENT)) || /*	need it for the MIP-maps when
                                             mipmaps required and not GL
                                             mipmaps required and supported
                                           */
      (iwidth >
       ...
       max_supported_size)) /*	2^n for later down-sampling)	*/
  {
    ...
    if ((new_width != iwidth) || (new_height != iheight)) {
      ...
      up_scale_image(NULL != img ? img : data, iwidth, iheight, channels,
                     resampled, new_width, new_height);

      /*	nuke the old guy ( if a copy exists ), then point it at the new
       * guy	*/
      SOIL_free_image_data(img);
      img = resampled;
      *width = new_width;
  ...
  if ((iwidth > max_supported_size) || (iheight > max_supported_size)) {
    ...
    mipmap_image(NULL != img ? img : data, iwidth, iheight, channels, resampled,
  ...
  if (tex_id) {
    ...
    if (flags & SOIL_FLAG_COMPRESS_TO_DXT) {
      ...
      if (DXT_mode == SOIL_CAPABILITY_PRESENT) {
        ...
        if ((channels & 1) == 1) {
          ...
          internal_texture_format = sRGB_texture
                                        ? SOIL_GL_COMPRESSED_SRGB_S3TC_DXT1_EXT
                                        : SOIL_RGB_S3TC_DXT1;
        } else {
          /*	2 or 4 channels = DXT5	*/
          internal_texture_format =
              sRGB_texture ? SOIL_GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT
                           : SOIL_RGBA_S3TC_DXT5;
        }
      }
    ...
    if (DXT_mode == SOIL_CAPABILITY_PRESENT) {
      ...
      if ((channels & 1) == 1) {
        /*	RGB, use DXT1	*/
        DDS_data = convert_image_to_DXT1(NULL != img ? img : data, iwidth,
                                         iheight, channels, &DDS_size);
      } else {
        /*	RGBA, use DXT5	*/
        DDS_data = convert_image_to_DXT5(NULL != img ? img : data, iwidth,
                                         iheight, channels, &DDS_size);
      }
      if (DDS_data) {
        ...
        /*	my compression failed, try the OpenGL driver's version	*/
        glTexImage2D(opengl_texture_target, 0, internal_texture_format, iwidth,
                     iheight, 0, original_texture_format, GL_UNSIGNED_BYTE,
                     NULL != img ? img : data);
        check_for_GL_errors("glTexImage2D");
    ...
    if (flags & SOIL_FLAG_MIPMAPS || flags & SOIL_FLAG_GL_MIPMAPS) {
      createMipmaps(NULL != img ? img : data, iwidth, iheight, channels, flags,
                    opengl_texture_target, internal_texture_format,
                    original_texture_format, DXT_mode);
...
int SOIL_save_image(const char *filename, int image_type, int width, int height,
  ...
  return SOIL_save_image_quality(filename, image_type, width, height, channels,
                                 data, 80);
}
...
int SOIL_save_image_quality(const char *filename, int image_type, int width,
  ...
  if (image_type == SOIL_SAVE_TYPE_BMP) {
    ...
    save_result = stbi_write_png(filename, width, height, channels,
                                 (const unsigned char *const)data, 0);
  } else if (image_type == SOIL_SAVE_TYPE_JPG) {
    save_result = jo_write_jpg(filename, (const void *)data, width, height,
                               channels, quality);
  } else {
    save_result = 0;
  }
...
void SOIL_free_image_data(unsigned char *img_data) {
  if (img_data) free((void *)img_data);
}
...
unsigned int SOIL_direct_load_DDS(const char *filename,
  ...
  tex_ID = SOIL_direct_load_DDS_from_memory(
      (const unsigned char *const)buffer, (int)buffer_length, reuse_texture_ID,
...
unsigned int SOIL_direct_load_PVR_from_memory(const unsigned char *const buffer,
  ...
  unsigned int opengl_texture_type =
      loading_as_cubemap ? SOIL_TEXTURE_CUBE_MAP : GL_TEXTURE_2D;
  int is_PVRTC_supported = query_PVR_capability() == SOIL_CAPABILITY_PRESENT;
  int is_BGRA8888_supported =
      query_BGRA8888_capability() == SOIL_CAPABILITY_PRESENT;
  ...
  switch (header->dwpfFlags & PVRTEX_PIXELTYPE) {
    ...
    case OGL_PVRTC2:
      if (is_PVRTC_supported) {
        ...
        PVR_format = header->dwAlphaBitMask == 0
                         ? SOIL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG
                         : SOIL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG;  // PVRTC2
      } else {
        result_string_pointer =
            "error: PVRTC2 not supported.Decompress the texture first.";
        return 0;
      }
      break;
    case MGLPT_PVRTC4:
    case OGL_PVRTC4:
      if (is_PVRTC_supported) {
        is_compressed_format_supported = is_compressed_format = 1;
        PVR_format = header->dwAlphaBitMask == 0
                         ? SOIL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG
                         : SOIL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG;  // PVRTC4
      } else {
        result_string_pointer =
            "error: PVRTC4 not supported. Decompress the texture first.";
        return 0;
      }
      break;
    case OGL_BGRA_8888:
      if (is_BGRA8888_supported) {
        ...
        result_string_pointer =
            "Unable to load GL_BGRA texture as extension "
            "GL_IMG_texture_format_BGRA8888 is unsupported.";
        return 0;
      }
...
#define _MAX(a, b) ((a <= b) ? b : a)
  for (i = 0; i < num_surfs; i++) {
    char *texture_ptr =
        (char *)buffer + header->dwHeaderSize + header->dwTextureDataSize * i;
    ...
    mipmaps =
        ((flags & SOIL_FLAG_MIPMAPS) && (header->dwpfFlags & PVRTEX_MIPMAP))
            ? header->dwMipMapCount
            : 0;

    for (mipmap_level = 0; mipmap_level <= mipmaps;
         width = _MAX(width / 2, (unsigned int)1),
        height = _MAX(height / 2, (unsigned int)1), mipmap_level++) {
      ...
      if (is_compressed_format) {
        ...
        if ((header->dwpfFlags & PVRTEX_PIXELTYPE) == OGL_PVRTC2) {
          compressed_image_size =
              (_MAX(width, PVRTC2_MIN_TEXWIDTH) *
                   _MAX(height, PVRTC2_MIN_TEXHEIGHT) * header->dwBitCount +
               7) /
              8;
        } else {  // PVRTC4 case
          compressed_image_size =
              (_MAX(width, PVRTC4_MIN_TEXWIDTH) *
                   _MAX(height, PVRTC4_MIN_TEXHEIGHT) * header->dwBitCount +
               7) /
              8;
        }
        ...
        if (is_compressed_format_supported) {
          ...
          if (loading_as_cubemap) {
            soilGlCompressedTexImage2D(
                SOIL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mipmap_level, PVR_format,
                width, height, 0, compressed_image_size, cur_texture_ptr);
          } else {
            soilGlCompressedTexImage2D(opengl_texture_type, mipmap_level,
                                       PVR_format, width, height, 0,
                                       compressed_image_size, cur_texture_ptr);
          }
        } else {
          result_string_pointer =
              "failed: GPU doesnt support compressed textures";
        }
      } else {
        /* Load uncompressed texture data at selected MIP level */
        if (loading_as_cubemap) {
          glTexImage2D(SOIL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mipmap_level,
                       PVR_type, width, height, 0, PVR_type, PVR_format,
                       cur_texture_ptr);
        } else {
          glTexImage2D(opengl_texture_type, mipmap_level, PVR_type, width,
                       height, 0, PVR_type, PVR_format, cur_texture_ptr);
        }
      }
      ...
      if (is_compressed_format) {
        ...
        /* New formula that takes into account bit counts inferior to 8 (e.g. 1
         * bpp) */
...
unsigned int SOIL_direct_load_ETC1_from_memory(
  ...
  if (query_ETC1_capability() != SOIL_CAPABILITY_PRESENT) {
    result_string_pointer =
        "error: ETC1 not supported. Decompress the texture first.";
    return 0;
  }
  ...
  soilGlCompressedTexImage2D(opengl_texture_type, 0, SOIL_GL_ETC1_RGB8_OES,
                             width, height, 0, compressed_image_size,
                             texture_ptr);
...
int query_tex_rectangle_capability(void) {
  ...
  if (has_tex_rectangle_capability == SOIL_CAPABILITY_UNKNOWN) {
    /*	we haven't yet checked for the capability, do so	*/
    if ((0 == SOIL_GL_ExtensionSupported("GL_ARB_texture_rectangle")) &&
...
int query_cubemap_capability(void) {
  ...
  if (has_cubemap_capability == SOIL_CAPABILITY_UNKNOWN) {
    /*	we haven't yet checked for the capability, do so	*/
    if ((0 == SOIL_GL_ExtensionSupported("GL_ARB_texture_cube_map")) &&
...
#if defined(SOIL_PLATFORM_WIN32) || defined(SOIL_PLATFORM_OSX) || \
    defined(SOIL_X11_PLATFORM)
  ext_addr = (P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC)SOIL_GL_GetProcAddress(
...
int query_DXT_capability(void) {
  ...
  if (has_DXT_capability == SOIL_CAPABILITY_UNKNOWN) {
    /*	we haven't yet checked for the capability, do so	*/
    if (0 == SOIL_GL_ExtensionSupported("GL_EXT_texture_compression_s3tc") &&
        0 == SOIL_GL_ExtensionSupported("WEBGL_compressed_texture_s3tc ") &&
        0 == SOIL_GL_ExtensionSupported(
                 "WEBKIT_WEBGL_compressed_texture_s3tc") &&
        0 == SOIL_GL_ExtensionSupported("MOZ_WEBGL_compressed_texture_s3tc")) {
      /*	not there, flag the failure	*/
      has_DXT_capability = SOIL_CAPABILITY_NONE;
    } else {
      P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC ext_addr =
          get_glCompressedTexImage2D_addr();
...
int query_PVR_capability(void) {
  ...
  if (has_PVR_capability == SOIL_CAPABILITY_UNKNOWN) {
    /*	we haven't yet checked for the capability, do so	*/
    if (0 == SOIL_GL_ExtensionSupported("GL_IMG_texture_compression_pvrtc")) {
      /*	not there, flag the failure	*/
...
int query_BGRA8888_capability(void) {
  ...
  if (has_BGRA8888_capability == SOIL_CAPABILITY_UNKNOWN) {
    /*	we haven't yet checked for the capability, do so	*/
    if (0 == SOIL_GL_ExtensionSupported("GL_IMG_texture_format_BGRA8888")) {
      /*	not there, flag the failure	*/
...
int query_ETC1_capability(void) {
  ...
  if (has_ETC1_capability == SOIL_CAPABILITY_UNKNOWN) {
    /*	we haven't yet checked for the capability, do so	*/
    if (0 ==
        SOIL_GL_ExtensionSupported("GL_OES_compressed_ETC1_RGB8_texture")) {
      /*	not there, flag the failure	*/
...
int query_gen_mipmap_capability(void) {
  ...
  if (has_gen_mipmap_capability == SOIL_CAPABILITY_UNKNOWN) {
    if (0 == SOIL_GL_ExtensionSupported("GL_ARB_framebuffer_object") &&
      ...
      ext_addr = (P_SOIL_GLGENERATEMIPMAPPROC)SOIL_GL_GetProcAddress(
          "glGenerateMipmap");
