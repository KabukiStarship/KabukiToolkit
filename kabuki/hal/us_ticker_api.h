 ...
 *
 * # Defined behavior
 * * Has a reported frequency between 250KHz and 8MHz - Verified by test
 * ::us_ticker_info_test
 * * Has a counter that is at least 16 bits wide - Verified by test
 * ::us_ticker_info_test
 ...
 *
 * # Defined behavior
 * * The function ticker_init is safe to call repeatedly - Verified by test
 * ::ticker_init_test
 * * The function ticker_init allows the ticker to keep counting and disables
 * the ticker interrupt - Verified by test ::ticker_init_test
 * * Ticker frequency is non-zero and counter is at least 8 bits - Verified by
 * ::ticker_info_test
 * * The ticker rolls over at (1 << bits) and continues counting starting from 0
 * - Verified by ::ticker_overflow_test
 ...
 * * The ticker interrupt fires only when the ticker times increments to or past
 * the value set by ticker_set_interrupt. Verified by ::ticker_interrupt_test
 * and ::ticker_past_test
 * * It is safe to call ticker_set_interrupt repeatedly before the handler is
 * called - Verified by ::ticker_repeat_reschedule_test
 * * The function ticker_fire_interrupt causes ticker_irq_handler to be called
 * immediately from interrupt context - Verified by ::ticker_fire_now_test
 * * The ticker operations ticker_read, ticker_clear_interrupt,
 * ticker_set_interrupt and ticker_fire_interrupt take less than 20us to
 * complete - Verified by ::ticker_speed_test
 *
 * # Undefined behavior
 * * Calling any function other than ticker_init before the initialization of
 * the ticker
 * * Whether ticker_irq_handler is called a second time if the time wraps and
 * matches the value set by ticker_set_interrupt again
 * * Calling ticker_set_interrupt with a value that has more than the supported
 * number of bits
 ...
 * @note this function is primarily for testing purposes and it's not required
 * part of HAL implementation
 *
 */
ticker_irq_handler_type set_us_ticker_irq_handler(
    ticker_irq_handler_type ticker_irq_handler);
