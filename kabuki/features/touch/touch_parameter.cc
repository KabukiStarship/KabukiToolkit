...
@file    /kabuki/features/touch/touch_parameter.cc
...
Parameter::Parameter(uint16_t type, const CH1* label, uint16_t channel,
...
SI4 Parameter::compare(const Parameter& o) const {
...
const CH1* Parameter::GetTypeString() const {
  switch (type_) {
    case ControlDmx:
      return "DMX Control";
    case MidiControl:
