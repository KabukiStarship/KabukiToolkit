...
*/
template<typename T>
INLINE Array<T>* ArrayInit (byte* buffer, uint_t type, T n)
{
...
/** Inserts the item into the array at the given index.
    ...
    @param */
template<typename T>
INLINE int Insert (Array<T>* a, T item, int index) {
    if (a == nullptr)
        return -1;
    int count = a->count;
    if (count <= 0)
...
template<typename T>
INLINE int Add (Array<T>* a, int item) {
    return Insert (a, item, a->count - 1);
}

/** Removes the given index from the array. */
template<typename T>
INLINE int Remove (Array<T>* a, T index) {
    if (a == nullptr)
        return !((T)0);
    int count = a->count;
    if (count == 0)
