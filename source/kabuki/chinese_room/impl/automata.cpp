...
KABUKI MonoidRx* BInit (byte* buffer, uint_t size) {
    if (buffer == nullptr)
        return nullptr;
    MonoidRx* rx = reinterpret_cast<MonoidRx*>(rx);
...
KABUKI MonoidRx* BInit (MonoidRx* rx, uint_t size) {
    if (rx == nullptr)
        return nullptr;
    rx->size = size - sizeof (MonoidRx);
    rx->start = 0;
...
KABUKI byte* BBaseAddress (MonoidRx* ptr) {
    return reinterpret_cast <byte*>(ptr) + kSlotHeaderSize;
}

KABUKI uint_t BSpace (MonoidRx* rx) {
...
KABUKI byte* BEndAddress (MonoidRx* rx) {
...
KABUKI bool IsReadable (MonoidRx* rx) {
    return rx->start != rx->stop;
}

KABUKI void Print (MonoidRx* rx) {
