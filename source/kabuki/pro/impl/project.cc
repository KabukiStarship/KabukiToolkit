...
bool Project::Select (int index)
{
    ...
    if (index >= schedules_.GetCount ()) return false;
    task_ = schedules_[index];
    return true;
}
...
void Project::ListObjects ()
{
    ...
    std::cout << "|\n"
                 "| Objects in Scope:\n"
                 "|\n"
                 "| Projects: " << projects_.GetCount () << '\n';
    for (int i = 0; i < projects_.GetCount (); ++i)
        std::cout << "| " << i << ".) " << projects_[i]->GetKey () << '\n';
    std::cout << "|\n"
                 "| Schedules: " << schedules_.GetCount () << '\n';
    for (int i = 0; i < schedules_.GetCount (); ++i)
        std::cout << "| " << i << ".) " << schedules_[i]->GetKey () << '\n';
    PrintBreak ("|", '_');
}
...
bool Project::AddSchedule (const char* key)
{
    if (!IsToken (key))
        return false;
    schedules_.Add (new Schedule (key));
    return true;
}
...
int Project::ScheduleIndex (const char* key)
{
    ...
    for (int i = 0; i < schedules_.GetCount (); ++i)
        if (CompareTokenString (key, schedules_[i]->GetKey ()))
            return i;
    return -1;
}
...
{
    return schedules_.Remove (index);
}
...
int Project::ProjectIndex (const char* key)
{
    ...
    //  Search for an already existing key to add it to:
    for (int i = 0; i < projects_.GetCount (); ++i)
        if (CompareTokenString (key, projects_[i]->GetKey ()))
            return i;
    return -1;
}
...
{
    return projects_[ProjectIndex (key)];
}

bool Project::RemoveProject (int index)
{
    return projects_.Remove (index);
}
...
const char* Project::GetAppHelpString ()
{
    return "\n| Kabuki::_pro::Project Help:\n"
           ...
           "|            Schedule(s) and a collection of sub-projects.\n"
           "|\n"
           ...
           "|\n"
           "| Examples: subproject example-schedule -s \"A task\" -b 3:30PM'\n"
           "|           class-schedule -a 1.0 -s \"Homework\" -d \"Program 5\n"
...
void Project::Print (int indent_level, char bullet_type, int spaces_per_tab)
{
    ...
    PrintBreak ("\n|");
    std::cout << "| Projects: " << projects_.GetCount ()
              << "|\n";
    for (int i = 0; i < projects_.GetCount (); ++i)
    {
        projects_[i]->Print ();
    }
    std::cout << "| Schedules: " << schedules_.GetCount ()
              << "|\n";
    for (int i = 0; i < schedules_.GetCount (); ++i)
    {
        schedules_[i]->Print ();
...
const char* Project::Command (char* input, ProjectNode*& stack)
{
    ...
    else if (isdigit (c))
    {
        ...
        std::cout << " >" << key_ << " >" << input << '\n';
        input = EndOfToken (input);      // Scroll past the number.
        int num_subprojects = projects_.GetCount ();
        if (value < num_subprojects)
            return projects_[value]->Command (input, stack);
        else if (value < num_subprojects + schedules_.GetCount () - 1)
    ...
    if (*input == 0)
        return Push (stack, this);
    // Search for key to pass the command string onto.
    end = EndOfToken (input);
    std::cout << " >" << key_ << " >" << input << '\n';
    if (*input == 0)
    ...
    value = ProjectIndex (input);
    if (value >= 0)
        return projects_[value]->Command (end, stack);
    value = ScheduleIndex (input);
    if (value >= 0)
        return schedules_[value]->Command (end);
