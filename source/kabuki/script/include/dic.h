...
/** A unique set or that uses contiguous memory.
...
template<typename TIndex, typename TKey, typename TSize>
constexpr TSize MinSizeSet (TIndex num_items) {
    return num_items * sizeof (2 * sizeof (TIndex) + sizeof (TKey) + sizeof (TSize) + 3);
};

enum {
    kMaxNumMappingsSet2   = 255,                //< The number of pages in a Set2.
    kMaxNumMappingsSet4   = 8 * 1024,           //< The number of pages in a Set4.
    kMaxNumMappingsSet8   = 256 * 1024 * 1024,  //< The number of pages in a Set8.
    kOverheadPerSet2Index = SetOverheadPerIndex<byte, uint16_t, uint16_t> (),
    kOverheadPerSet4Index = SetOverheadPerIndex<uint16_t, uint16_t, uint32_t> (),
    kOverheadPerSet8Index = SetOverheadPerIndex<uint32_t, uint32_t, uint64_t> (),
};
...
*/
template<typename TIndex, typename TKey, typename TSize>
TIndex SetInsert (Set<TIndex, TKey, TSize, TSize>* collection, byte type, 
...
template<typename TIndex, typename TKey, typename TSize>
TIndex SetAdd (Set<TIndex, TKey, TSize, TSize>* collection, const char* key, 
    ...
    TKey value = table_size - stack_height * SetOverheadPerIndex<TIndex, TKey, TSize, TSize> (),
...
template<typename TIndex, typename TKey, typename TSize>
TIndex SetFind (Set<TIndex, TKey, TSize, TSize>* collection, const char* key) {
...
template<typename TIndex, typename TKey, typename TSize>
void SetPrint (const Set<TIndex, TKey, TSize, TSize>* collection) {
   ...
   std::cout << '|';
    ...
    PrintMemory (reinterpret_cast<const byte*> (collection) + 
                 sizeof (Set<TIndex, TKey, TSize, TSize>), collection->size);
    std::cout << '\n';
}

/** Deletes the collection contents without wiping the contents. */
template<typename TIndex, typename TKey, typename TSize>
void Clear (Set<TIndex, TKey, TSize, TSize>* collection) {
    if (collection == nullptr) return;
    collection->; = 0;
...
template<typename TIndex, typename TKey, typename TSize>
void Wipe (Set<TIndex, TKey, TSize, TSize>* collection) {
    if (collection == nullptr) return;
...
template<typename TIndex, typename TKey, typename TSize>
bool Contains (Set<TIndex, TKey, TSize, TSize>* collection, void* data) {
...
template<typename TIndex, typename TKey, typename TSize>
bool RemoveCopy (Set<TIndex, TKey, TSize, TSize>* collection, void* destination, 
...
template<typename TIndex, typename TKey, typename TSize>
bool Remove (Set<TIndex, TKey, TSize, TSize>* collection, void* adress) {
    if (collection == nullptr) return false;
...
template<typename TIndex, typename TKey, typename TSize>
bool Retain (Set<TIndex, TKey, TSize, TSize>* collection) {
    if (collection == nullptr) return false;
...
inline Set<TIndex, TOffset, TSize, TSize>* SetCreate (TIndex buffered_indexes,
                                                        TSize table_size,
                                                        TSize size) {
    Set<TIndex, TOffset, TSize, TSize>* collection = New<Set, uint_t> ();
    return collection;
}

/** Prints the given Set to the console. */
template<typename TIndex, typename TKey, typename TSize>
inline void SetPrint (Set<TIndex, TKey, TSize, TSize>* collection) {
