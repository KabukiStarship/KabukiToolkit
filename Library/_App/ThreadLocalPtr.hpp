...
#include <ChineseRoom/ThreadLocal.hpp>
...
/*< Pointer to a thread-local variable
    ...
    _::ThreadLocalPtr is a type-safe wrapper for storing pointers to 
    thread-local variables. A thread-local variable holds a different value for 
    each different thread, unlike normal variables that are shared.
    ...
    MyClass object1;
    MyClass object2;
    _::ThreadLocalPtr<MyClass> objectPtr;
    ...
    ThreadLocalPtr is designed for internal use; however you can use it if you 
    feel like it fits well your implementation. */
{
    public:
