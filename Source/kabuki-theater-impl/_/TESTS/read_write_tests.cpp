/** Kabuki Theater
    @version 0.x
    @file    /.../source/kabuki-theater-impl/_/TESTS/read_write_tests.cpp
...
{
    void setup () {
        PrintLineBreak ("+ Running ReadWriteTests...", 10);
    }
...
TEST (ReadWriteTests, TestingPrimitiveTypes) {
    ...
    Unityper* tx = TxInit (Buffer<0, 128 + kSlotHeaderSize> (), 128 + kSlotHeaderSize);
    ...
    CHECK_EQUAL (0, Write (tx, Esc<4, SI2, SI2, UI2, HLF> (),
           Args (args, &si2_p_expected, &si2_n_expected, &ui2_expected,
                 &hlf_expected)))
    CHECK_EQUAL (0, Read (tx, Esc<4, SI2, SI2, UI2, HLF> (),
          Args (args, &si2_p_found, &si2_n_found, &ui2_found, &hlf_found)))
    ...
    CHECK_EQUAL (0, Write (tx, Esc<5, SI4, SI4, UI4, FLT, TMS> (),
           Args (args, &si4_p_expected, &si4_n_expected, &ui4_expected,
                 &flt_expected, &tms_expected)))
    ...
    int16_t sv2_found[7];
    CHECK_EQUAL (0, Write (tx, Esc<7, SV2, SV2, SV2, SV2, SV2, SV2, SV2> (),
    ...
    CHECK_EQUAL (0, Write (tx, Esc<4, UV2, UV2, UV2, UV2> (),
    ...
    CHECK_EQUAL (0, Write (tx, Esc<11, SV4, SV4, SV4, SV4, SV4, SV4, SV4, SV4,
                                   SV4, SV4, SV4> (),
    ...
    CHECK_EQUAL (0, Write (tx, Esc<6, UV4, UV4, UV4, UV4, UV4, UV4> (),
