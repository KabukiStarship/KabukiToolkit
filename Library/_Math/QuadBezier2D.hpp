...
/*< Class that represents a 4th order Bezier. */
{
    ...
    QuadBezier_f (const float* initXPoints, const float* initYPoints);
    /*< Constructor.
        @pre The length of XPoints and YPoints must equal NumPoints or curve will not be created correctly. */

    QuadBezier_f (float initX0, float initY0, float initX1, float initY1, float initX2, float initY2, float initX3, float initY3);
    /*< Constructs a bezier curve with the given points. */

    QuadBezier_f (const QuadBezier_f& o);
