...
@file    /kabuki/features/touch/touch_device_button.cc
...
DeviceButton::DeviceButton(const CH1 *label, MacroEvent *macro,
...
BOL ButtonMacro::GetFocus(TemplatePage *P) {
  if (!P) return false;
  focus_page_ = P;
  return true;
}
...
BOL ButtonMacro::GetTemplate(Template *T) {
  if (!T) return false;
  focus_template_ = T;
  return true;
}
