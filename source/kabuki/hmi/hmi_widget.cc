/** Kabuki Toolkit
    @file    ~/source/kabuki/hmi/hmi_widget.cc
...
bool Widget::GetName (const char* name)
{// Renames this Widget to name
...
int Widget::insertDuplicate (const char* name)
{
    ...
    name = name.dropLastCharacters (String (trailingValue).length ()) + String (++trailingValue);
    return insertDuplicate (name);
}



int Widget::insertPage (const char* name)
...
WidgetPage* Widget::page (const char* name)
{
    ...
    do {
        if (name.compare (current->getLabel ()))
            return current;
        current = current->next ();
    } while (current != pagesHead);
