...
@file    /kabuki/features/touch/touch_button_page.cc
...
namespace _ {
  ...
  const Op* PageButton::op(_::Terminal * io, SI4 index) {
    switch (index) {
      case 0:
        return Script::NumMembers(0);
    }

    return Query ? Enquery("PageButton", "kabuki::touch::Buttons")
                 : InvalidIndex();
  }

  const CH1* PageButton::Print(Script::Terminal & slot) {
    const CH1* returnString = "Page Button: \n";
