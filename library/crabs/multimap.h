...
/** A multismap created from contiguous memory.
    ...
    A multimap is like a Python dictionary or C++ map, the difference being a TMultimap
    can contain nested TMultimap (string). The key design difference between both Python 
    dictionaries and C++ maps are Sets do not contains points, and instead
    works using offsets.

    A multimap may or may not have a hash table. In order to turn on the hash table,
    simply set the collissionsSize to non-zero in the TMultimap header.

    The memory layout is the same for all of the TMultimap types as depicted below:
...
constexpr uint_t MultimapOverheadPerIndex () {
        return sizeof (2 * sizeof (I) + sizeof (SI) + 
               sizeof (UI) + 3);
};
...
constexpr UI MultimapSizeMin (I item_count) {
    return item_count * sizeof (2 * sizeof (I) + sizeof (SI) + 
           sizeof (UI) + 3);
};
...
uintptr_t* MultimapInit (uintptr_t* buffer, byte count_max, uint16_t size) {
    ASSERT (buffer);
    if (table_size < sizeof (TMap) + count_max *
        (MultimapOverheadPerIndex<byte, uint16_t, uint16_t, uint16_t> () + 2))
        return nullptr;
...
*/
template<typename UI, typename SI, typename I>
I MultimapInsert (TMap<SI, I>* multimap, byte type, 
               const char* key, void* data, I index) {
...
I MultimapIndexMax () {
    enum {
        kMaxIndexes = sizeof (I) == 1 ? 255 : sizeof (I) == 2 ? 
                       8 * 1024 : sizeof (I) == 4 ? 512 * 1024 * 1024 : 0
    };
    return kMaxIndexes;
}
...
template<typename UI, typename SI, typename I>
I MultimapAdd (TMap<SI, I>* multimap, const char* key, 
    ...
    I item_count = multimap->item_count,
        count_max = multimap->count_max,
        temp;
    ...
    char* states = reinterpret_cast<char*> (multimap) + 
                   sizeof (TMap <SI, I>);
    SI* key_offsets = reinterpret_cast<SI*> (states + count_max);
    UI* data_offsets = reinterpret_cast<UI*> (states + count_max *
                                                    (sizeof (SI)));
    UI* hashes = reinterpret_cast<UI*> (states + count_max *
                                              (sizeof (SI) + sizeof (UI))),
        * hash_ptr;
    I* indexes = reinterpret_cast<I*> (states + count_max *
                                                 (sizeof (SI) + sizeof (UI) + sizeof (I))),
        *unsorted_indexes = indexes + count_max,
        *collission_list = unsorted_indexes + count_max;
    char* keys = reinterpret_cast<char*> (multimap) + table_size - 1,
        *destination;
        ...
        } else    // Duplicate hash detected.
        {
            ...
            if (index < ~0)             //< There are other collisions.
            {
                ...
                *temp_ptr = item_count;
                ...
                multimap->pile_size = pile_size + 1;
                PRINTF ("\n\ncollision index: %u\n", temp);
            ...
            if (strcmp (key, keys - key_offsets[index]) != 0) {
                ...
                *temp_ptr = item_count;
                ++temp_ptr;
                *temp_ptr = ~0;
                multimap->pile_size = pile_size + 3;
...
//    return SetAdd<byte, uint16_t, uint16_t, uint16_t> (multimap, key, UI1, &data);
//}
...
I MultimapFind (TMap<SI, I>* multimap, const char* key) {
    ...
    I index,
    item_count = multimap->item_count,
        count_max = multimap->count_max,
        temp;
    ...
    const UI* hashes = reinterpret_cast<const UI*>
        (reinterpret_cast<const char*> (multimap) +
         sizeof (TMap<SI, I>));
...
void MultimapPrint (const TMap<SI, I>* multimap) {
    ...
    PRINTF ("\n;: %u count_max: %u  "
            ...
            count_max, pile_size, table_size);
    PRINTF ('\n';
    PRINTF ('|';
...
template<typename UI, typename SI, typename I>
TMap<SI, I>* MultimapCreate (I buffered_indexes,
                                           UI table_size,
                                           UI size) {
...
class Multimap {
    ...
    */
    inline I Insert (byte type, const char* key, void* data, I index) {
        ...
        return MultimapInsert<SI, I> (multimap, type, key, data, 
                                                   index);
    }

    inline I IndexMax () {
        return MultimapIndexMax<SI, I> ();
    }
    ...
    /** Removes that object from the multimap and copies it to the destination. */
    inline bool MultimapRemoveCopy (void* destination, size_t buffer_size, void* data) {
        auto multimap = reinterpret_cast<Multimap<SI, I>> (buffer_);
        return;
    }
